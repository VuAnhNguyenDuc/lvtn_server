    ...
    public ResponseEntity<Message> loginApi(@ModelAttribute("user") User user, Errors errors, Model model){
        if(user.getUsername() != null && user.getPassword() != null){
            ...
            if(result != null){
                ...
                return new ResponseEntity<>(new Message("Login successful", jsonKey), HttpStatus.OK);
            } else{
                return new ResponseEntity<>(new Message("Incorrect username or password", ""), HttpStatus.OK);
            }
        }
        return new ResponseEntity<>(new Message("Username and Password cannot be empty!!", ""), HttpStatus.NO_CONTENT);
    }
}
